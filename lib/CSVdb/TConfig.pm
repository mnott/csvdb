package CSVdb::TConfig;

=pod

=head1 NAME

CSVdb::TConfig - Read and hold shared configuration information
...
use CSVdb::TUtils qw/ t_as_string /;
...
my $MODULE       = 'CSVdb::TConfig';

my @DEPENDENCIES = qw / CSVdb::TMain
                        CSVdb::TConfig
                        CSVdb::TUtils
                        CSVdb::TFileResolver
                      /;
...
=item I<Set>
    ...
    $cfg->clear();             # Assume we did CSVdb::TConfig->new();
    $cfg->set("a", "b");
    my $res = $cfg->get("a");  # "b"
...
Example:
    ...
    $cfg->load("t/cvsdb.ini", {'protect_global' => 0});
